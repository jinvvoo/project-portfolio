...
| :---: | :---: | :---: | :---: |
| 합성 데이터 | `Mask` / `No Mask` | 탐지 장치 / 모니터링 장치 | TCP 이벤트 / imagezmq 영상 |
...
| 항목 | 내용 |
| --- | --- |
| 기간 | 2021.03 ~ 2021.12 *(학위논문 제출 2022.02)* |
| 구분 | 4인 졸업작품·졸업논문 |
...
학습 원본에는 AI로 생성된 얼굴 이미지를 사용했습니다. 얼굴의 방향과 안경 착용 여부가 다른 이미지를 수집하고, 실제 사람의 개인정보 노출을 줄이면서 학습 데이터를 확장했습니다.
...
실험 단계에서는 Keras CNN을 학습하고, 최종 시연에서는 실시간 추론에 맞춘 모델을 사용했습니다.

### 실험용 CNN

실험 단계의 Keras `Sequential` 모델은 다음 순서로 특징을 학습합니다.
...
최종 시연 모델에서는 MobileNetV2 계열 구조를 사용했습니다. 논문 실험용 CNN과 최종 시연 모델은 서로 다른 단계의 결과로 구분해 정리했습니다.
...
<p align="center">
  ...
  <br />
  <sub>80 epoch 학습 로그: 학습·검증 손실 및 정확도 추이</sub>
</p>

이 그래프는 80 epoch 동안 학습·검증 손실과 정확도가 변화한 과정을 보여줍니다.

| 시연 항목 | 결과 |
| --- | --- |
| 흰색 마스크 | 데이터를 추가한 뒤 시연 프레임에서 `Mask 99%` 표시 |
| 검은색 마스크 | 검은색 마스크 데이터를 추가한 뒤 시연 프레임에서 `Mask 99%` 표시 |
| 코 노출 착용 | 별도 데이터를 학습한 뒤 시연 프레임에서 `No Mask 99%` 표시 |
| 최종 시연 | 흰색·검은색 사례 모두 표시 신뢰도 91% 이상 |

표의 백분율은 **각 시연 프레임에서 모델이 출력한 신뢰도**입니다.
